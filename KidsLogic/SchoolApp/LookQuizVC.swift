...
class LookQuizVC: UIViewController , AVSpeechSynthesizerDelegate, UICollectionViewDataSource, UICollectionViewDelegate,UICollectionViewDelegateFlowLayout{

        var passArrDataSelected: [[String: Any]]?
        var tempPassArrDataSelected: [[String: Any]]?
        var correctAns: String?
        var rnd: UInt?
        var arrAnsOption: [Any]?
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = GlobleConstants.MAIN_CAT_LOOK
        ...
        self.quizLookCollection.register(collectionNib, forCellWithReuseIdentifier: "CategoryCollectionViewCell")

        switch selectedSubCat {
        case 0:
           alphabetDetailsArray()
        case 1: numberDetailsArray()
        case 2: colorDetailsArray()
        case 3: shapesDetailsArray()
        case 4: animalDetailsArray()
        case 5: birdsDetailsArray()
        case 6: flowerDetailsArray()
        case 7: fruitDetailsArray()
        case 8: monthDetailsArray()
        case 9: vegetableDetailsArray()
        case 10: bodyPartsDetailsArray()
        case 11: clothesDetailsArray()
        case 12: countryDetailsArray()
        case 13: foodDetailsArray()
        case 14: geometryDetailsArray()
        case 15: houseDetailsArray()
        case 16: jobsDetailsArray()
        ...
        rnd = UInt(arc4random_uniform(UInt32(passArrDataSelected?.count ?? 0)))
        DisplaySelcte(indexVal: Int(rnd ?? 0))
    ...
    func DisplaySelcte(indexVal: Int) {
        guard let passArrDataSelected = passArrDataSelected, indexVal < passArrDataSelected.count else {
            return
        }

        let imageName = passArrDataSelected[indexVal]["Image"] as? String ?? ""
        imgSelected.image = UIImage(named: imageName)
        imgSelected.layer.cornerRadius = 10
        imgSelected.clipsToBounds = true
        
        arrAnsOption = [Any]()
        if let title = passArrDataSelected[indexVal]["Title"] as? String {
            arrAnsOption?.append(title)
            correctAns = title
        }
        ...
        for _ in 0..<3 {
            let randomIndex = Int(arc4random_uniform(UInt32(tempPassArrDataSelected?.count ?? 0)))
            if let title = tempPassArrDataSelected?[randomIndex]["Title"] as? String {
                arrAnsOption?.append(title)
            }
            tempPassArrDataSelected?.remove(at: randomIndex)
        }

        arrAnsOption?.sort { ($0 as? String ?? "").localizedCaseInsensitiveCompare($1 as? String ?? "") == .orderedAscending }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let strTitle = arrAnsOption![indexPath.row]
        cell.lblName.text = strTitle as! String

        var finalCellFrame = cell.lblName.frame
        ...
        UIView.animate(withDuration: 0.8) {
            cell.lblName.frame = finalCellFrame
        }

        if UIDevice.current.userInterfaceIdiom == .phone {
            cell.lblName.font = UIFont.boldSystemFont(ofSize: 16)
        } else {
            cell.lblName.font = UIFont.boldSystemFont(ofSize: 30)
        }
        return cell
    }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let strTitleSelect = arrAnsOption![indexPath.row]
        ...
        if correctAns! == strTitleSelect as? String {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        let cellSize = CGSize(width: self.quizLookCollection.frame.size.width / 2, height: self.quizLookCollection.frame.size.height / 2)
        return cellSize
    }
